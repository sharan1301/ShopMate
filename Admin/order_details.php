				...
				<div class="card-body--">
				   <div class="table-stats order-table ov-h">
					  <table class="table">
								...
								<tbody>
									<?php
									$res=mysqli_query($con,"select distinct(order_detail.id) ,order_detail.*,product.name,product.image,orders.block_no,orders.address,orders.city,orders.state,orders.pincode from order_detail,product ,orders where order_detail.order_id='$order_id' and  order_detail.product_id=product.id GROUP by order_detail.id");
									$total_price=0;
									
									$userInfo=mysqli_fetch_assoc(mysqli_query($con,"select * from orders where id='$order_id'"));
									
									$block_no=$userInfo['block_no'];
						...
						<div id="address_details">
							<strong style="margin:15px;">Address : </strong>
							<?php echo $block_no?>,<?php echo $address?>, <?php echo $city?>,<?php echo $state?>, <?php echo $pincode?><br/><br/>
							<strong style="margin:15px;">Order Status : </strong>
							<?php 
							...
							<div style="margin:15px;">
								<form method="post">
									...
									<input style="margin-top:15px;" type="submit" class="form-control"/>
								</form>
							</div>
						</div>
				   </div>
